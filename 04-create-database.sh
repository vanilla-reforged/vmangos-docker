...
docker exec -it vmangos_database sudo mariadb -u root -p$MYSQL_ROOT_PASSWORD -e "CREATE DATABASE IF NOT EXISTS mangos DEFAULT CHARSET utf8 COLLATE utf8_general_ci;"
docker exec -it vmangos_database sudo mariadb -u root -p$MYSQL_ROOT_PASSWORD -e "CREATE DATABASE IF NOT EXISTS logs DEFAULT CHARSET utf8 COLLATE utf8_general_ci;"

echo "[VMaNGOS]: Creating user…"
docker exec -it vmangos_database sudo mariadb -u root -p$MYSQL_ROOT_PASSWORD -e "CREATE USER 'mangos'@'localhost' IDENTIFIED BY '$MYSQL_ROOT_PASSWORD';"
docker exec -it vmangos_database sudo mariadb -u root -p$MYSQL_ROOT_PASSWORD -e "SET PASSWORD FOR 'mangos'@'localhost' = PASSWORD('$MYSQL_ROOT_PASSWORD');"

echo "[VMaNGOS]: Granting privileges for user…"
docker exec -it vmangos_database sudo mariadb -u root -p$MYSQL_ROOT_PASSWORD -e "GRANT ALL PRIVILEGES ON *.* TO 'mangos'@'%' IDENTIFIED BY '$MYSQL_ROOT_PASSWORD';"
docker exec -it vmangos_database sudo mariadb -u root -p$MYSQL_ROOT_PASSWORD -e "FLUSH PRIVILEGES;"
docker exec -it vmangos_database sudo mariadb -u root -p$MYSQL_ROOT_PASSWORD -e "GRANT ALL ON realmd.* TO mangos@'localhost' WITH GRANT OPTION;"
docker exec -it vmangos_database sudo mariadb -u root -p$MYSQL_ROOT_PASSWORD -e "GRANT ALL ON characters.* TO mangos@'localhost' WITH GRANT OPTION;"
docker exec -it vmangos_database sudo mariadb -u root -p$MYSQL_ROOT_PASSWORD -e "GRANT ALL ON mangos.* TO mangos@'localhost' WITH GRANT OPTION;"
docker exec -it vmangos_database sudo mariadb -u root -p$MYSQL_ROOT_PASSWORD -e "GRANT ALL ON logs.* TO mangos@'localhost' WITH GRANT OPTION;"
docker exec -it vmangos_database sudo mariadb -u root -p$MYSQL_ROOT_PASSWORD -e "FLUSH PRIVILEGES;"
